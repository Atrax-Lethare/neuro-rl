"""Accuracy of the *trained* Qwen3-1.7B-Instruct + LoRA adapter on the held-out scenario pool.

Usage (after Kaggle training run finishes and adapter is pushed to HF Hub):
    HF_USER=<your-hf-username> python scripts/eval_trained.py

Environment variables:
    HF_USER            HuggingFace username — adapter loaded from $HF_USER/neuro-rl-adapter
    ADAPTER_REPO       Override full adapter repo (e.g. org/my-adapter); takes precedence over HF_USER
    EVAL_BASE_MODEL    Base model to load (default: Qwen/Qwen3-1.7B-Instruct)

Outputs:
    outputs/trained_metrics.json     — accuracy table (overall, per-intent, per-drift)
    outputs/drift_resistance.json    — 30-point drift sweep for intent=move_left

Acceptance criteria:
    trained_metrics.json  overall_accuracy ≥ 0.70
    drift_resistance.json contains exactly 30 (phase, accuracy) pairs
"""
...
# Drift-resistance sweep parameters
...
# ---------------------------------------------------------------------------
# Inference helper (shared by both eval and drift sweep)
# ---------------------------------------------------------------------------
...
def load_model_with_adapter(base_model_name: str, adapter_repo: str):
    ...
    if not adapter_repo:
        raise SystemExit(
            ...
            "  Set HF_USER=<your-username>  or  ADAPTER_REPO=<org/repo>\n"
            "  and ensure the Kaggle training run has pushed the adapter to HF Hub."
        )
    ...
    print(f"Loaded.  Total params: {n_params:.2f}B  Adapter trainable: {n_trainable:.1f}M\n")
...
def run_eval(model, tokenizer, device: str) -> dict:
    ...
    print(f"Evaluating {total} held-out scenarios...\n")
    ...
    out_path = outputs_dir / "trained_metrics.json"
    with open(out_path, "w") as fh:
        json.dump(metrics, fh, indent=2)

    print("\n" + "=" * 60)
    print(f"TRAINED  —  {BASE_MODEL_NAME} + {ADAPTER_REPO}")
    print("=" * 60)
    print(f"Overall accuracy : {overall_accuracy:.1%}  ({correct}/{total})")
    print(f"  (target ≥ 70%;  baseline random-guess ≈ 14.3%)\n")
...
def run_drift_sweep(model, tokenizer, device: str) -> list[dict]:
    ...
    Returns a list of 30 dicts: {drift_phase, accuracy, correct, total, phase_label}.
    Saved to outputs/drift_resistance.json.
    """
    ...
    for phase_idx, drift_phase in enumerate(phases):
        ...
        for ep in range(DRIFT_SWEEP_EPISODES_PER_PHASE):
            # Unique seed per (phase_idx, episode) keeps episodes independent
    ...
    outputs_dir = _ROOT / "outputs"
    outputs_dir.mkdir(exist_ok=True)
    out_path = outputs_dir / "drift_resistance.json"
    with open(out_path, "w") as fh:
        json.dump(
            {
                "intent": DRIFT_SWEEP_INTENT,
...
if __name__ == "__main__":
    model, tokenizer, device = load_model_with_adapter(BASE_MODEL_NAME, ADAPTER_REPO)
