...
"""
NeuroRLEnv – OpenEnv §3.5 environment implementation.

Simulates a non-stationary neural decoding task: the agent observes Poisson
spike trains generated from one of seven motor intents and must decode the
ground-truth intent at each step.
"""
...
class NeuroRLEnv(Environment):
    ...
    def step(self, action: NeuroRLAction) -> NeuroRLObservation:
        ...
        # done when max_steps reached OR 5 consecutive correct decodes
        ...
        # Re-sample spike train for the SAME intent (single-window decoding)
        self._noise_level = float(self._rng.uniform(5.0, 20.0))
        spikes, mfr, disc = generate_spike_train(
            self._current_intent,
            noise_level=self._noise_level,
